function vecLoc = linformlocherm1d(fName, element, iDegree, ...
...
for iShape = 1:(iDegree + 3) % +1 to +3
    if (iShape == 1) || (iShape == 2)
        integrand = feval(fName,gaussPoint).*shapeloc1dherm(gaussPoint, ...
            element, iDegree, iDerivative, iShape);
        vecLoc(iShape) = sum(gaussWeight.*integrand);
    end
    if (iShape == 3) || (iShape == 4)
        integrand = jaco*feval(fName,gaussPoint).*shapeloc1dherm(gaussPoint, ...
            element, iDegree, iDerivative, iShape);
        vecLoc(iShape) = sum(gaussWeight.*integrand);
    end
end