function [uDof, nodeType] = bc_array_generator_1d(Fem, domain, iBoundaryCondition)
...
for i = 1:length(Fem.point)
    if (abs(Fem.point(i) - domain(1)) <= tolerance) && (i <= length(Fem.point)/2)
        nodeType(i) = iBoundaryCondition(1);
    elseif (abs(Fem.point(i) - domain(2)) <= tolerance) && (i <= length(Fem.point)/2)
        nodeType(i) = iBoundaryCondition(2);
    elseif (abs(Fem.point(i) - domain(1)) <= tolerance) && (i > length(Fem.point)/2)
        nodeType(i) = iBoundaryCondition(3);
    elseif (abs(Fem.point(i) - domain(2)) <= tolerance) && (i > length(Fem.point)/2)
        nodeType(i) = iBoundaryCondition(4);
    else
        nodeType(i) = -200;
    end
end
