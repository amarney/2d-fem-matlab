function matLoc = biformlocherm1d(aName, element, iDegree1, iDerivative1, ...
    iDegree2, iDerivative2, nGaussPoint)
...
for iShape1 = 1:(iDegree1+3) % +1 to +3
    for iShape2 = 1:(iDegree2+3) % +1 to +3
        ...
        integrand = feval(aName,gaussPoint).*shapeloc1dherm(gaussPoint,element,iDegree1,iDerivative1,iShape1).*...
        ...
        integrand = jaco*feval(aName,gaussPoint).*shapeloc1dherm(gaussPoint,element,iDegree1,iDerivative1,iShape1).*...
            shapeloc1dherm(gaussPoint,element,iDegree2,iDerivative2,iShape2);
        matLoc(iShape1,iShape2) = sum(gaussWeight.*integrand);            
        end
        ...
        integrand = jaco*jaco*feval(aName,gaussPoint).*shapeloc1dherm(gaussPoint,element,iDegree1,iDerivative1,iShape1).*...
            shapeloc1dherm(gaussPoint,element,iDegree2,iDerivative2,iShape2);
        matLoc(iShape1,iShape2) = sum(gaussWeight.*integrand);            
        end        
    end
